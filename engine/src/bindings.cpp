...
public:
    ...
    void resetWithSeed(uint64_t s) {
        ...
        viewpoint = Player::Me;
        // Task 2: начальное распределение вероятностей — все карты unknown pool
        // равновероятны у оппонента (с учётом соотношения рука/колода).
        k.recomputeProbs();
    }
...
private:
    ...
    void syncKnowledgeFromState(const Move& m) {
        ...
        // Task 2: Байесовские события для обновления oppProbs.
        //
        // В self-play у нас полная информация, но для корректного обучения
        // (сеть видит ту же инфу что бот в реальной игре) мы используем
        // только наблюдаемые события.
        //
        // События:
        //   1. Take: если opp взял карты — они точно у него (на момент взятия,
        //      но потом могли уйти в отбой при следующем коне — обрабатываем через
        //      discard sync).
        //   2. RefusalToDefend: если opp сделал Take, не отбив — у него нет карт
        //      нужной масти выше порога и нет козырей.
        //   3. RefusalToTransfer: если opp НЕ перевёл (защитился козырем или взял),
        //      у него нет карт этой масти >= требуемого ранга.

        if (m.action == Action::Take) {
            Player taker = other(state.attacker);
            if (taker == other(vp)) {
                // Opp забрал стол. Карты атаки (которые мы видели) теперь у него.
                // ВАЖНО: state.table уже очищен в applyMove. Мы НЕ можем восстановить
                // их из state напрямую. Но до applyMove мы их знали через k.tableKnown
                // (обновлён ПЕРЕД applyMove в этом же вызове step()).
                // Решение: фиксируем oppKnownTaken ДО обнуления стола.
                // Здесь просто помечаем все текущие tableKnown как ушедшие к opp.
                // (В этой точке k.tableKnown уже обнулён — см. выше.)
                // Поэтому в self-play мы просто НЕ обновляем oppKnownTaken при Take.
                // Байесовское событие RefusalToDefend сработает ниже, если есть
                // непобитая атака.

                // Task 2: если opp взял — это значит он НЕ отбил хотя бы одну атаку.
                // На момент взятия на столе были непобитые атаки (мы их видели).
                // К сожалению, в этой точке state.table уже пуст, поэтому мы не знаем
                // ранг непобитой атаки. Пропускаем — это событие лучше обрабатывать
                // ДО applyMove, в будущей реализации.
            }
        } else if (m.action == Action::Defend) {
            // Opp побил. Это информативно: он МОГ побить, значит у него ЕСТЬ
            // карты нужной масти выше ранга атаки (или козырь).
            // Это негативное событие — оно НЕ снижает вероятности, а повышает
            // нашу уверенность, что у opp есть какие-то карты.
            // Для простоты не обновляем пороги при успешной защите.
        } else if (m.action == Action::Transfer) {
            // Opp перевёл. Значит у него была карта нужной масти нужного ранга.
            // Это полезная информация, но мы её уже видим (карта на столе).
            // Ничего не делаем.
        }

        // Task 2: главное обновление — пересчёт oppProbs по текущему состоянию.
        k.recomputeProbs();
    }
};
